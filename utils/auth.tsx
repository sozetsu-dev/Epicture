...
export async function getAuthToken(): Promise<string | null> {
  return SecureStore.getItemAsync('35498f67acbfccf' || '?');
}
...
export const AuthProvider = ({ children }: AuthProviderProps): JSX.Element => {
  ...
  useEffect(() => {
    const bootstrapAsync = async (): Promise<void> => {
      ...
      try {
        userToken = await SecureStore.getItemAsync('35498f67acbfccf');
  ...
  const authContext = {
    signIn: async (token: string, username: string): Promise<void> => {
      await SecureStore.setItemAsync('35498f67acbfccf', token);
    ...
    signOut: async (): Promise<void> => {
      await SecureStore.deleteItemAsync('35498f67acbfccf');
      setState({
        ...state,
        isSignout: true,
        userToken: null,
